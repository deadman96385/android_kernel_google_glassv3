...
#ifdef A254_I2C_DBG
#define a254_err(fmt, args...) pr_err(fmt, ##args)
#else
#define a254_err(fmt, args...)
#endif
...
#define RESET_HIGH_PIN 89
    ...
    pr_err("disp drv id:0x%x, chip_rst:0x%x\n", chip->disp_drv_id, chip->en_a254_reset_low);

    return rc;
}

int a254_send_receive(uint8_t command, uint8_t value, uint8_t *data, int data_len)
{
    ...
    if (data != 0)
    {
        pr_err("A254 I2C Read !!!\n");
        msgs[0].len = 1;		//andy0x, for read, msg[0].len needs to change to 1
        msgs[1].addr = 0x34;
...
static int a254_regulator_hw_init(struct a254_chip *chip)
{
    ...
    if (chip->a254_pwr1p2) {
        if (regulator_count_voltages(chip->a254_pwr1p2) > 0) {
            rc = regulator_set_voltage(chip->a254_pwr1p2,
                1350000, 1350000);
            if (rc < 0) {
    ...
    if (chip->a254_pwr1p8) {
        if (regulator_count_voltages(chip->a254_pwr1p8) > 0) {
            rc = regulator_set_voltage(chip->a254_pwr1p8,
                1800000, 1800000);
            if (rc < 0) {
...
static int a254_parse_dt(struct a254_chip *chip,
                struct i2c_client *client)
{
    ...
    chip->en_pwr1p2_gpio = of_get_named_gpio(client->dev.of_node,
                          "a254,1p2-gpio", 0);

    chip->a254_reset_gpio = of_get_named_gpio(client->dev.of_node,
                          "a254,reset-gpio", 0);

    chip->en_reset_high_detect = of_property_read_bool(client->dev.of_node,
                          "a254,reset-high-detect");

    if (of_find_property(client->dev.of_node, "a254-1p2-supply", NULL)) {
        chip->a254_pwr1p2 = devm_regulator_get(&client->dev, "a254-1p2");
    ...
    if (of_find_property(client->dev.of_node, "a254-1p8-supply", NULL)) {
        chip->a254_pwr1p8 = devm_regulator_get(&client->dev, "a254-1p8");
    ...
    if (of_find_property(client->dev.of_node, "a254-4p5-supply", NULL)) {
        chip->a254_pwr4p5 = devm_regulator_get(&client->dev, "a254-4p5");
    ...
    if (of_find_property(client->dev.of_node, "a254-6p0-supply", NULL)) {
        chip->a254_pwr6p0 = devm_regulator_get(&client->dev, "a254-6p0");
...
static int a254_probe(struct i2c_client *client,
            const struct i2c_device_id *id)
{
    struct a254_chip *chip;
    int rc=0;

    chip = devm_kzalloc(&client->dev, sizeof(struct a254_chip),
                            GFP_KERNEL);
    if (!chip) {
...
static const struct i2c_device_id a254_id[] = {
    { "a254", 0 },
    { }
};
